...
#    `functionName` is a function's name in your module (e.g., `vanWagnerEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/stevec/Dropbox/Courses/7043H16/Lab/Harvesting/vanWagner/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("vanWagner")
  ...
  parameters <- list(
    ...
    vanWagner = list(.saveInitialTime = NA)
  )
  ...
  if (exists("vanWagnerEvent1", envir = .GlobalEnv)) {
    simOutput <- vanWagnerEvent1(mySim)
  } else {
    simOutput <- mySim$vanWagnerEvent1(mySim)
  }
  ...
  if (exists("vanWagnerEvent2", envir = .GlobalEnv)) {
    simOutput <- vanWagnerEvent2(mySim)
  } else {
    simOutput <- mySim$vanWagnerEvent2(mySim)
  }
